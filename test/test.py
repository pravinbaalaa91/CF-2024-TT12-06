...
async def test_reset_and_initial_state(dut):
    ...
    await reset_dut(dut)
    cocotb.log.info(f"[test_reset_and_initial_state] uo_out[0]={dut.uo_out[0].value.integer}, uo_out[1]={dut.uo_out[1].value.integer}")
...
async def test_pwm_zero_duty_cycle(dut):
    ...
    for cycle in range(260):
        await RisingEdge(dut.clk)
        cocotb.log.info(f"[test_pwm_zero_duty_cycle] cycle={cycle}, uo_out[0]={dut.uo_out[0].value.integer}, uo_out[1]={dut.uo_out[1].value.integer}")
...
async def test_pwm_full_duty_cycle(dut):
    ...
    for cycle in range(260):
        await RisingEdge(dut.clk)
        cocotb.log.info(f"[test_pwm_full_duty_cycle] cycle={cycle}, uo_out[0]={dut.uo_out[0].value.integer}, uo_out[1]={dut.uo_out[1].value.integer}")
...
async def test_pwm_mid_duty_cycle(dut):
    ...
    for cycle in range(total_cycles):
        ...
        cocotb.log.info(f"[test_pwm_mid_duty_cycle] cycle={cycle}, uo_out[0]={val}")
        high_count += val
    cocotb.log.info(f"[test_pwm_mid_duty_cycle] high_count={high_count} after {total_cycles} cycles")
...
async def test_pwm_out_and_pwm_out1_equal(dut):
    ...
    for duty in [0, 10, 50, 100]:
        ...
        cocotb.log.info(f"[test_pwm_out_and_pwm_out1_equal] duty={duty}")
        await Timer(5000, units="ns")
        for cycle in range(10):
            await RisingEdge(dut.clk)
            val0 = dut.uo_out[0].value.integer
            val1 = dut.uo_out[1].value.integer
            cocotb.log.info(f"[test_pwm_out_and_pwm_out1_equal] cycle={cycle}, uo_out[0]={val0}, uo_out[1]={val1}")
...
async def test_pwm_edge_cases(dut):
    ...
    dut.ui_in.value = 1
    cocotb.log.info("[test_pwm_edge_cases] duty=1")
    await Timer(3000, units="ns")
    # Upper boundary just below 100
    dut.ui_in.value = 99
    cocotb.log.info("[test_pwm_edge_cases] duty=99")
    await Timer(3000, units="ns")
